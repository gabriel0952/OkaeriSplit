-- Drop first because the return type changes (adding currency column).
DROP FUNCTION IF EXISTS get_overall_balances(UUID);

-- Update get_overall_balances to:
-- 1. Apply exchange rate conversion (matching get_user_balances logic)
-- 2. Return the group's base currency so the client doesn't need to guess
